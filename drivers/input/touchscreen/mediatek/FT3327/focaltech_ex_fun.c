...
static ssize_t fts_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (fts_i2c_read_reg(client, FTS_REG_FW_VER, &fwver) < 0) {
        num_read_chars = snprintf(buf, PAGE_SIZE, "Read fwer, I2c transfer error!\n");
    }
#if (FTS_GET_VENDOR_ID_NUM > 1)
    if(fts_fwupg_get_vendorid(ts_data, &vendor_id)){
         num_read_chars += snprintf(buf, PAGE_SIZE, "Read vendor id, I2c transfer error!\n");
    }
#endif
