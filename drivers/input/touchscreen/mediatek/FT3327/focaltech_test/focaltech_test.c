...
/**
* Add focal tp INT & Reaset PIN test
*/
static int fts_pin_test(void)
{
	...
	ret = gpio_request(tpd_dts_data.tpd_int_gpio, "gpio_int_std");
#else
	ret = gpio_request(tpd_dts_data.tpd_int_gpio, "gpio_int");
#endif
	if(ret != 0){
		FTS_ERROR("request int gpio errori!");
		FTS_TEST_SAVE_INFO_RESULT("TPD INT GPIO=%d\n", tpd_dts_data.tpd_int_gpio);
		return -1;
	}
	...
	if (fts_i2c_read_reg(client, FTS_REG_WORKMODE, &mode) < 0) {
		...
		if((mode & 0x70) != FTS_REG_WORKMODE_FACTORY_VALUE) {
			...
			ret = -1;
			ret = gpio_direction_input(tpd_dts_data.tpd_int_gpio);
			if(ret != 0){
				FTS_ERROR("INI GPIO direction input error!");
			}
			irq_value = gpio_get_value(tpd_dts_data.tpd_int_gpio);
			fts_i2c_write_reg(client, 0x08, 0x01);
			msleep(10);
			if(irq_value != gpio_get_value(tpd_dts_data.tpd_int_gpio))
			{
				FTS_INFO("===irq gpio test ok\n");
				FTS_TEST_SAVE_INFO_RESULT("irq gpio test ok\n");
				ret = 0;
			}
			else
			{
				FTS_INFO("===irq gpio test failed\n");
				FTS_TEST_SAVE_INFO_RESULT("irq gpio test failed!\n");
				ret = -1;
				goto irq_test_failed;
			}
	...
	if (reg_value != org_value) {
		...
		FTS_TEST_SAVE_INFO_RESULT("reset test failed!\n");
		ret = -1;
		goto reset_test_failed;
	} else {
		FTS_INFO("===rest gpio test ok\n");
		FTS_TEST_SAVE_INFO_RESULT("rest gpio test ok\n");
		ret = 0;
	}
...
irq_test_failed:
	...
	gpio_free(tpd_dts_data.tpd_int_gpio);
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if(fts_pin_test() == 0)
    {
	    FTS_TEST_SAVE_INFO_RESULT("Pin test pass.\n");
	    FTS_INFO("===Pin test pass.\n");
    }
    else
    {
	    FTS_TEST_SAVE_INFO_RESULT("Pin test failure.\n");
