...
void tpd_get_dts_info(void)
{
	...
	int key_dim_local[16] = {0}, i = 0, ret;;

	node1 = of_find_matching_node(node1, touch_of_match);
	if (node1) {
		tpd_dts_data.tpd_rst_gpio = -1;
		tpd_dts_data.tpd_int_gpio = -1;
#if (!defined(CONFIG_MTK_GPIO) || defined(CONFIG_MTK_GPIOLIB_STAND))
		ret = of_get_named_gpio(node1, "gpio-tp-rst-std", 0);
		if(ret < 0){
			TPD_DEBUG("get gpio-tp-rst-std error ret=%d!\n", ret);
		}else{
			tpd_dts_data.tpd_rst_gpio = ret;
		}
		
		ret = of_get_named_gpio(node1, "gpio-tp-int-std", 0);
		if(ret < 0){
			TPD_DEBUG("get gpio-tp-int-std error!\n");
		}else{
			tpd_dts_data.tpd_int_gpio = ret;
		}
#else
		ret = of_property_read_u32_array(node1, "gpio-tp-rst", &(tpd_dts_data.tpd_rst_gpio), 1);
		if(ret != 0){
			TPD_DEBUG("Not Found gpio-tp-rst\n");
		}
		ret = of_property_read_u32_array(node1, "gpio-tp-int", &(tpd_dts_data.tpd_int_gpio), 1);
		if(ret != 0){
			TPD_DEBUG("Not Found gpio-int-rst\n");
		}
#endif
		TPD_DEBUG("firefly mtk-tpd get gpio rst=%d, int=%d\n", tpd_dts_data.tpd_rst_gpio, tpd_dts_data.tpd_int_gpio);
