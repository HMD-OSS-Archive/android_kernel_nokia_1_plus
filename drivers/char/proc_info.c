...
static char causeStr[99] = {'\0'};
static char *fver_preload = NULL;
static int fver_len = 65536;
static int fver_open_times = 0;
static char *sutinfo_preload = NULL;
static int sutinfo_len = 256;
static int sutinfo_open_times = 0;
static char sim_state='\0';
static char str_imei[20] = {'\0'};
static char str_imei2[20] = {'\0'};
static char str_pid[33] = {'\0'};
static char str_uicolor[20] = {'\0'};
...
static char fih_emmc_info[20] = {'\0'};
static char fih_emmc_vendor[10] = {'\0'};
static char fih_emmc_size[5] = {'\0'};
static char fih_dram_info[20] = {'\0'};
static char fih_dram_vendor[10] = {'\0'};
static char fih_dram_size[5] = {'\0'};

extern u8 mem_cid[9];
extern char fih_pid[33];
static unsigned long long mtk_emmc_size = 0;

static char *root_status_load = NULL;
static int root_status_len = 4096;
static bool open_already = 0;

int devmodel_init = 0;
static char str_project[16] = {'\0'};
static int sim_card_slot = 0;
...
static int sutinfo_show(struct seq_file *s, void *unused)
{
	struct file *sutinfo_filp = NULL;
	mm_segment_t oldfs = 0;
	loff_t pos = 0;
	int iCount = 0;

	//
	if( ( 0 != sutinfo_open_times) && ( NULL != sutinfo_preload) )
	{
		seq_printf(s, "%s\n", sutinfo_preload);
		return 0;
	}
	...
	//first open
	if(NULL == sutinfo_preload)
	{
		do{
			sutinfo_preload = kmalloc(sizeof(char)*sutinfo_len, GFP_KERNEL);
			if( sutinfo_preload != NULL )
			{
				printk("alloc memory for sutinfo successfull!\n");
				break;
			}
			else{
				printk("alloc memory for sutinfo fail!\n");
				iCount ++;
				//return -ENOMEM;
			}
		}while( (iCount <= 5) && (NULL == sutinfo_preload) ) ;

		if( (iCount > 5) ||  ( NULL == sutinfo_preload) )
		{
			printk("alloc memory for sutinfo fail!\n");
			return -ENOMEM;
		}
	...
	oldfs = get_fs();
	set_fs(KERNEL_DS);
	sutinfo_filp = filp_open(sutinfo_BLOCK, O_RDONLY, 0);


	if(!IS_ERR(sutinfo_filp))
	{
		iCount = 0;
		iCount = vfs_read(sutinfo_filp, (char __user *)sutinfo_preload, sizeof(char)*sutinfo_len, &pos);
		filp_close(sutinfo_filp, NULL);
		sutinfo_open_times++;

		printk("read sutinfo successfully %d !\n",iCount);
	}
	else
	{
		printk("[dw]open %s fail\n", sutinfo_BLOCK);
	...
	//kfree(sutinfo_preload);
	//sutinfo_filp = sutinfo_preload = NULL;
	return 0;
}
...
static int lcm_show(struct seq_file *s, void *unused)
{
	...
	if (!strcmp(fih_lcm_info, "st7701_fwvgaplus_dsi_vdo_kotl"))
	{
		seq_printf(s, "ST7701-KOTL 5.45 IPS Oncell Fwvgplus TFT\n");
	}else if (!strcmp(fih_lcm_info, "st7701_fwvgaplus_dsi_vdo_hlt")){
		seq_printf(s, "ST7701-HLT 5.45 IPS Oncell Fwvgplus TFT\n");
	}else{
		seq_printf(s, "unknow\n");
	}
...
static int sim_card_slot_show(struct seq_file *s,void *unused)
{
	...
	//FIH Houwuyang 2018/08/13 set sim_card_slot start.
	...
	return 0;
	//FIH Houwuyang 2018/08/13 set sim_card_slot end.
}
...
static int hwid_info_show(struct seq_file *s, void *unused)
{
	...
	printk("project_id = %d, phase_id = %d, module_id = %d\n", project_id, phase_id, module_id);
...
static int sutinfo_open(struct inode *inode, struct file *file)
{
	return single_open(file, sutinfo_show, &inode->i_private);
}
...
static const struct file_operations sutinfo_fops = {
        .open        = sutinfo_open,
        .read        = seq_read,
        .llseek      = seq_lseek,
        .release     = single_release,
...
#define ERROR_PATH -300
s32 fih_check_fs_mounted(char *path_name)
{
...
*/
		while (fih_check_fs_mounted("/vendor/etc"))
		{
			printk("vendor filesystem is not ready!\n");
			msleep(100);
		}

		printk("vendor filesystem is ready!\n");

		fdata_filp = filp_open(FIH_HWID_INFO, O_RDONLY, 0);

		if(!IS_ERR(fdata_filp))
		{
			//result = fdata_filp->f_op->llseek(fdata_filp, 0, SEEK_SET);
			//result = fdata_filp->f_op->read(fdata_filp, hwid_tag, FIH_HWID_TAG_SIZE, &fdata_filp->f_pos);
...
int fih_read_hwid_info(void *x)
{
	int i = 0;
	char fih_hwid_num = 0;
	char *temp_strptr = NULL;
#define CMDLINE_TMP_CONCAT_SIZE 200
	char cmdline_tmpbuf[CMDLINE_TMP_CONCAT_SIZE];

	struct fih_hwid_info *hwid_info_tabel = NULL;
	struct fih_hwid_info *pHtable = NULL;
	...
	if (hw_project > 4)
	{
		printk("Illegal project number(%d > 4)\n",hw_project);
		return -1;
	}

	fih_hwid_num = fs_get_hwid_num();
	printk("fih_read_hwid_info() fih_hwid_num = %d,prj=%d,phase=%d,module=%d\n", fih_hwid_num,hw_project,hw_phase,hw_module);
	...
	if(fih_hwid_num > 0)
	{
		...
		if (!hwid_info_tabel)
		{
			...
			memcpy(&model[hw_project], &unknown_model, sizeof(struct systeminfo));

			printk("fih_read_hwid_info() kmalloc Failed!\n");

			return -1;
		}
		pHtable = hwid_info_tabel;

		memset(hwid_info_tabel, 0, sizeof(struct fih_hwid_info)*fih_hwid_num);

		fs_read_hwid_info((char *)pHtable, FIH_HWID_TAG_SIZE*2, sizeof(struct fih_hwid_info)*fih_hwid_num);

		for(i = 0; i < fih_hwid_num; i++, pHtable++)
		{
			if((pHtable->project_id == hw_project) &&
				(pHtable->phase_id == hw_phase) && (pHtable->module_id == hw_module))
			{
				strcpy(model[hw_project].model_name, pHtable->project_name);
				strcpy(model[hw_project].cpu_name, pHtable->cpu_name);
				strcpy(model[hw_project].phase_settings, pHtable->phase_sw);
				strcpy(model[hw_project].phase, pHtable->phase_hw);
				strcpy(model[hw_project].module, pHtable->module);
				strcpy(model[hw_project].bandinfo, pHtable->bandinfo);
				strcpy(model[hw_project].pcba_description, pHtable->pcba_description);
				strcpy(model[hw_project].hw_family, pHtable->hw_family);
				strcpy(model[hw_project].hac, pHtable->hac);
				strcpy(model[hw_project].sim_num, pHtable->sim_num);
				break;
			}
		}

		kfree(hwid_info_tabel);
		hwid_info_tabel = pHtable = NULL ;

		if(i == fih_hwid_num)
		{
			printk("Cant find matched project,just fill it with unkown\n");
			memset(&model[hw_project], 0, sizeof(struct systeminfo));
	...
	snprintf(cmdline_tmpbuf, CMDLINE_TMP_CONCAT_SIZE, "%s%s", " androidboot.hwmodel=", model[hw_project].model_name);

	strncat(saved_command_line, cmdline_tmpbuf, strlen(cmdline_tmpbuf) + 1);
	//kfree(temp_strptr);

	if(0 == strncmp(model[hw_project].sim_num, "Single", strlen("Single")))
	{
		sim_card_slot = 1;
	}
	else
	...
	snprintf(cmdline_tmpbuf, CMDLINE_TMP_CONCAT_SIZE, "%s%d", " androidboot.simslot=", sim_card_slot);
...
static int __init proc_info_module_init(void)
{
	...
	entry = proc_create(SUTINFO_PROC, S_IFREG | S_IRUGO, NULL, &sutinfo_fops);
	if(entry == NULL)
		printk("[dw]creat proc %s fail\n", SUTINFO_PROC);
	//sutinfo_preload = kmalloc(sizeof(char)*sutinfo_len, GFP_KERNEL);
	...
	entry = proc_create(TOUCH_INFO_1, 0777, entry_C, &touch_fops);
	if(entry == NULL)
		printk("creat touch proc %s fail\n", TOUCH_INFO);
	...
	if(entry == NULL)
        ...
        entry = proc_create(DRAM_PROC, 0777, entry_C, &dram_fops);
        if(entry == NULL)
                printk("creat AllHWList/dram proc %s fail\n", DRAM_PROC);
