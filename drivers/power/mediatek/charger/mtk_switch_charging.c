...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (info->enable_sw_jeita) {
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE) && info->chr_type == STANDARD_HOST)
			...
			pdata->input_current_limit = MIN(pdata->input_current_limit, info->sw_jeita.ichg);
			pdata->charging_current_limit = MIN(pdata->charging_current_limit, info->sw_jeita.ichg);
		}
	}
	...
	chr_debug("force:%d thermal:%d,%d pe4:%d,%d,%d setting:%d %d type:%d usb_unlimited:%d usbif:%d usbsm:%d aicl:%d\n",
