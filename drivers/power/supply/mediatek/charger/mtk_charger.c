...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	switch (sw_jeita->sm) {
		...
		sw_jeita->ichg = info->data.jeita_temp_above_t4_ichg_current;
		break;
	case TEMP_T3_TO_T4:
		sw_jeita->cv = info->data.jeita_temp_t3_to_t4_cv;
		sw_jeita->ichg = info->data.jeita_temp_t3_to_t4_ichg_current;
		break;
	case TEMP_T2_TO_T3:
		sw_jeita->cv = info->data.jeita_temp_t2_to_t3_cv;
		sw_jeita->ichg = info->data.jeita_temp_t2_to_t3_ichg_current;
		break;
	case TEMP_T1_TO_T2:
		sw_jeita->cv = info->data.jeita_temp_t1_to_t2_cv;
		sw_jeita->ichg = info->data.jeita_temp_t1_to_t2_ichg_current;
		break;
	case TEMP_T0_TO_T1:
		sw_jeita->cv = info->data.jeita_temp_t0_to_t1_cv;
		sw_jeita->ichg = info->data.jeita_temp_t0_to_t1_ichg_current;
		break;
	case TEMP_BELOW_T0:
		sw_jeita->cv = info->data.jeita_temp_below_t0_cv;
		sw_jeita->ichg = info->data.jeita_temp_below_t0_ichg_current;
		break;
	default:
...
#ifdef FIH_BUILDING_MODE_FACTORY_000F
	if(battery_get_bat_uisoc() >= 70) {
		charging = false;
		goto stop_charging;
	}
#endif
...
stop_charging:
	...
	chr_debug("tmp:%d (jeita:%d sm:%d cv:%d ichg:%d en:%d) (sm:%d) en:%d c:%d s:%d ov:%d sc:%d %d %d\n",
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "jeita_temp_above_t4_ichg_current", &val) >= 0) {
		info->data.jeita_temp_above_t4_ichg_current = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_ABOVE_T4_ICHG_CURRENT:%d\n", JEITA_TEMP_ABOVE_T4_ICHG_CURRENT);
		info->data.jeita_temp_above_t4_ichg_current = JEITA_TEMP_ABOVE_T4_ICHG_CURRENT;
	}

	if (of_property_read_u32(np, "jeita_temp_t3_to_t4_ichg_current", &val) >= 0) {
		info->data.jeita_temp_t3_to_t4_ichg_current = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_T3_TO_T4_ICHG_CURRENT:%d\n", JEITA_TEMP_T3_TO_T4_ICHG_CURRENT);
		info->data.jeita_temp_t3_to_t4_ichg_current = JEITA_TEMP_T3_TO_T4_ICHG_CURRENT;
	}

	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_ichg_current", &val) >= 0) {
		info->data.jeita_temp_t2_to_t3_ichg_current = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_T2_TO_T3_ICHG_CURRENT:%d\n", JEITA_TEMP_T2_TO_T3_ICHG_CURRENT);
		info->data.jeita_temp_t2_to_t3_ichg_current = JEITA_TEMP_T2_TO_T3_ICHG_CURRENT;
	}

	if (of_property_read_u32(np, "jeita_temp_t1_to_t2_ichg_current", &val) >= 0) {
		info->data.jeita_temp_t1_to_t2_ichg_current = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_T1_TO_T2_ICHG_CURRENT:%d\n", JEITA_TEMP_T1_TO_T2_ICHG_CURRENT);
		info->data.jeita_temp_t1_to_t2_ichg_current = JEITA_TEMP_T1_TO_T2_ICHG_CURRENT;
	}

	if (of_property_read_u32(np, "jeita_temp_t0_to_t1_ichg_current", &val) >= 0) {
		info->data.jeita_temp_t0_to_t1_ichg_current = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_T0_TO_T1_ICHG_CURRENT:%d\n", JEITA_TEMP_T0_TO_T1_ICHG_CURRENT);
		info->data.jeita_temp_t0_to_t1_ichg_current = JEITA_TEMP_T0_TO_T1_ICHG_CURRENT;
	}

	if (of_property_read_u32(np, "jeita_temp_below_t0_ichg_current", &val) >= 0) {
		info->data.jeita_temp_below_t0_ichg_current = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_BELOW_T0_ICHG_CURRENT:%d\n", JEITA_TEMP_BELOW_T0_ICHG_CURRENT);
		info->data.jeita_temp_below_t0_ichg_current = JEITA_TEMP_BELOW_T0_ICHG_CURRENT;
	}
