...
extern int g_fg_battery_id_volt;
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if((mt_get_charger_type() != CHARGER_UNKNOWN ) && (data->BAT_CAPACITY==100))
...
unsigned int TempConverBattThermistor(int temp)
{
	...
	if (temp >= Fg_Temperature_Table[131].BatteryTemp) {
		TBatt_R_Value = Fg_Temperature_Table[131].TemperatureR;
...
int BattThermistorConverTemp(int Res)
{
	...
	if (Res >= Fg_Temperature_Table[0].TemperatureR) {
		TBatt_Value = -400;
	} else if (Res <= Fg_Temperature_Table[131].TemperatureR) {
		TBatt_Value = 910;
	} else {
		RES1 = Fg_Temperature_Table[0].TemperatureR;
		TMP1 = Fg_Temperature_Table[0].BatteryTemp;
...
static ssize_t show_jeita_full_capacity(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", 0); /* fake value, only 1 or 2 is meanful */
}

static ssize_t store_jeita_full_capacity(
	struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	return size;
...
static DEVICE_ATTR(
	jeita_full_capacity, 0664,
	show_jeita_full_capacity, store_jeita_full_capacity);
