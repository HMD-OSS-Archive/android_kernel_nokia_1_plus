...
#if JUST_INPUT_NO_SWITCH
	switch_set_state((struct switch_dev *)&accdet_data, 0);//0:Not insert, 1:Key Not Pressed, 2:Key Pressed
#endif
...
static void accdet_work_callback(struct work_struct *work)
{
	...
	if (s_eint_accdet_sync_flag == 1){
		send_accdet_status_event(s_cable_type, 1);
#if JUST_INPUT_NO_SWITCH
        ACCDET_DEBUG("[accdet]cable type:[%s], status switch:[%s]\n", accdet_report_string[s_cable_type], accdet_status_string[s_accdet_status]);
        if(s_cable_type == NO_DEVICE)
                switch_set_state((struct switch_dev *)&accdet_data, 0);//0:Not insert, 1:Key Not Pressed, 2:Key Pressed
        else if(s_cable_type == HEADSET_NO_MIC)
                switch_set_state((struct switch_dev *)&accdet_data, 1);//0:Not insert, 1:Key Not Pressed, 2:Key Pressed
        else if(s_cable_type == HEADSET_MIC)
                switch_set_state((struct switch_dev *)&accdet_data, s_accdet_status);//0:Not insert, 1:Key Not Pressed, 2:Key Pressed
#endif
	}else
