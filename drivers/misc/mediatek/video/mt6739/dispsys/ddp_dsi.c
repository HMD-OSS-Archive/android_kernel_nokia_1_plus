...
long lcd_enp_bias_setting(unsigned int value)
{
	...
	if (value){
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_ENP_ENABLE);
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_ENN_ENABLE);
	}
	else{
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_ENP_DISABLE);
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BIAS_ENN_DISABLE);
	}
	return ret;
}
