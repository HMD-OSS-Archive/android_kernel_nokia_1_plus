...
#define S5K4H7SUB_AF_PAGE 22
#define S5K4H7SUB_INFO_PAGE 23

#define GOLD_R 464 // 464
#define GOLD_B 472 // 472
#define GOLD_GR 772 // 772
#define GOLD_GB 772 //772
...
typedef struct {
	...
	unsigned int	lsc_check_flag;
        ...
        unsigned short  af_infoflag;
        unsigned int    af_orientation;
        unsigned int    af_marco;
        unsigned int    af_infinity;
        unsigned int    af_check_sum;
        unsigned short  af_invaild;
}OTP;

typedef struct {
        int             lsc_group;
	unsigned int    lsc_check_flag;
        unsigned char   data_p_lsc[21][64];
}OTP_LSC;

OTP_LSC otp_data_lsc_info = {0};
OTP otp_data_info = {0};
...
int otp_group_info_4h7(void)
{
	...
	if( otp_data_info.lsc_infoflag == 0x01 ){
		...
		otp_data_info.lsc_group = 1;
                otp_data_lsc_info.lsc_group = 1;
		otp_data_info.lsc_sum = selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A30);
	}
	else if ( otp_data_info.lsc_infoflag == 0x03 ){
		otp_data_info.lsc_offset = 1;
		otp_data_info.lsc_group = 2;
                otp_data_lsc_info.lsc_group = 2;
	...
	otp_data_info.infoflag = selective_read_region(S5K4H7SUB_AWB_PAGE,S5K4H7SUB_OTP_AWBFLAG_ADDR);
        ...
        printk ("otp_data_info.infoflag awb : %d\n", otp_data_info.infoflag);

	otp_data_info.nrgolden = GOLD_R; //selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A16);
	otp_data_info.nbgolden = GOLD_B; //selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A18);
	otp_data_info.ngrgolden = GOLD_GR; //selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A1A);
	otp_data_info.ngbgolden = GOLD_GB;// selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A1c);
	...
	if( (otp_data_info.infoflag>>4 & 0x0c) == 0x04 ){
		...
		otp_data_info.awb_flag_sum = selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A2C);
		printk ("GOLD_AWB_RATIO page1 r_gr: %d b_gb: %d gr_gb: %d \n", selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A16),
                                  selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A18), selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A1A));
	}
	else if ( (otp_data_info.infoflag>>4 & 0x03) == 0x01 ){
		otp_data_info.awb_offset = 0X10;
		otp_data_info.group = 2;
		otp_data_info.nrcur = selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A1D);
		otp_data_info.nbcur = selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A1F);
		otp_data_info.ngrcur = selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A21);
		otp_data_info.ngbcur = selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A23);
		otp_data_info.awb_flag_sum = selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A2E);
		printk ("GOLD_AWB_RATIO page2 r_gr: %d b_gb: %d gr_gb: %d \n", selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A25),
				 selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A27), selective_read_region_16(S5K4H7SUB_AWB_PAGE,0x0A29));
	}
	else{
        ...
        printk ("cur %x %x %x %x  golden %x %x %x %x \n", otp_data_info.nrcur, otp_data_info.nbcur, otp_data_info.ngrcur, otp_data_info.ngbcur,
                        otp_data_info.nrgolden, otp_data_info.nbgolden, otp_data_info.ngrgolden, otp_data_info.ngbgolden);
	
	otp_data_info.af_infoflag = selective_read_region(S5K4H7SUB_AF_PAGE,0x0A04);
	if((otp_data_info.af_infoflag>>4 & 0x0c) == 0x04 ) {
		//af group 1
		otp_data_info.af_orientation = selective_read_region(S5K4H7SUB_AF_PAGE,0x0A05);
		otp_data_info.af_marco =  selective_read_region_16(S5K4H7SUB_AF_PAGE,0x0A06);
		otp_data_info.af_infinity = selective_read_region_16(S5K4H7SUB_AF_PAGE,0x0A08);
		otp_data_info.af_check_sum = selective_read_region_16(S5K4H7SUB_AF_PAGE,0x0A0F);
		
	} else if ( (otp_data_info.af_infoflag>>4 & 0x03) ==0x01 ) {
		//af group 2
		otp_data_info.af_orientation = selective_read_region(S5K4H7SUB_AF_PAGE,0x0A0A);
		otp_data_info.af_marco =  selective_read_region_16(S5K4H7SUB_AF_PAGE,0x0A0B);
                otp_data_info.af_infinity = selective_read_region_16(S5K4H7SUB_AF_PAGE,0x0A0D);
		otp_data_info.af_check_sum = selective_read_region_16(S5K4H7SUB_AF_PAGE,0x0A11);

	}else {
		printk("4h7 otp read data faied af info!!\n");
		goto error;
	}

	m_af_sum = otp_data_info.af_orientation + ((otp_data_info.af_marco >> 8) & 0xff) + (otp_data_info.af_marco & 0xff ) +
                                ((otp_data_info.af_infinity >> 8) & 0xff) + (otp_data_info.af_infinity & 0xff );
                if (m_af_sum == otp_data_info.af_check_sum) {
                        otp_data_info.af_invaild = 0x01;
                }else {
			printk("4h7 otp read af data check sum failed!! %d %d %d %d %d \n",otp_data_info.af_orientation,otp_data_info.af_marco,otp_data_info.af_infinity,otp_data_info.af_check_sum,m_af_sum);
			goto error;
		}
...
bool check_sum_flag_awb(void)
{
	...
	if(sum_awbfg == otp_data_info.awb_flag_sum){
                printk("check_sum_flag_awb, sum_awbfg:%d otp_data_info.awb_flag_sum: %d\n", sum_awbfg, otp_data_info.awb_flag_sum);
		apply_4h7_otp_awb();
	}
	else{
...
bool  check_sum_flag_lsc(void)
{
	...
	if(otp_data_info.lsc_group == 1){
		for(page_start = 1, page_end = 6; page_start <= page_end; page_start++){
			bresult &= read_4h7_page(page_start, page_start, otp_data_lsc_info.data_p_lsc[page_start-1]);
			if(page_start == 6){
				sum_slc += sum_awb_flag_lsc(0x0A04, 0x0A2B, otp_data_lsc_info.data_p_lsc[page_start-1]);
				continue;
			}
			sum_slc += sum_awb_flag_lsc(0x0A04, 0X0A43,otp_data_lsc_info.data_p_lsc[page_start-1]);
		}
	}
		...
		for(page_start = 6, page_end = 12; page_start <= page_end; page_start++){
			bresult &= read_4h7_page(page_start, page_start, otp_data_lsc_info.data_p_lsc[page_start-1]);
			if(page_start == 6){
				sum_slc += sum_awb_flag_lsc(0x0A2C, 0x0A43, otp_data_lsc_info.data_p_lsc[page_start-1]);
				continue;
			}
				...
				sum_slc += sum_awb_flag_lsc(0x0A04, 0X0A13, otp_data_lsc_info.data_p_lsc[page_start-1]);
			}
		}
	}
	...
	if(sum_slc == otp_data_info.lsc_sum){
		...
		otp_data_info.lsc_check_flag = 1;
                otp_data_lsc_info.lsc_check_flag = 1;
                printk("OTP 4h7 check lsc sum sum_slc:%d check_sum:%d d lsc_check_flag: %d\n", sum_slc, otp_data_info.lsc_sum, otp_data_info.lsc_check_flag);
	}
	else{
		printk("OTP 4h7 check lsc sum fail!!!");
		bresult &= 0;
		otp_data_info.lsc_check_flag = 0;
                otp_data_lsc_info.lsc_check_flag = 0;
	}
	return  bresult;
}
...
bool update_otp(void)
{
	...
	if(otp_group_info_4h7() == -1){
		...
		if(check_sum_flag_awb() == 0 ){
			printk("OTP 4h7 check awb sum fail!!!\n");
			result &= 0;
		}
		else{
			printk("OTP 4h7 check awb ok\n");
		}
#if 0
                if (otp_data_lsc_info.lsc_check_flag ==1 ) {
                       printk("OTP 4h7 check lsc ok\n");
                       apply_4h7_otp_enb_lsc();
                }else {
                      printk("OTP 4h7 check lsc fail\n");
                      result &= 0;
                }
#endif
	}
...
int GetAfCode_S5K4H7YX(u16 *Inf,u16 *Macro) 
{
if (otp_data_info.af_invaild) {
        *Inf = otp_data_info.af_infinity;
        *Macro = otp_data_info.af_marco;
        return 1;
    }
    else {
        *Inf = 200;
...
int GetLscCode_S5K4H7YX(u8 *lsc_data)
{
if ( otp_data_lsc_info.lsc_check_flag ) {
    if(otp_data_lsc_info.lsc_group == 1){
        memcpy(lsc_data, (u8*)otp_data_lsc_info.data_p_lsc, 360 );
    } else if ( otp_data_lsc_info.lsc_group ==2  ){
        memcpy(lsc_data,(u8*)otp_data_lsc_info.data_p_lsc + 360,360);
    }else{
        printk("s5k4h7 lsc error");
        return 0;
    }
}
    return 1;
...
int GetCurCode_S5K4H7YX(u16 *cur) {

if (sum_awbfg == otp_data_info.awb_flag_sum) {
    cur[0] = otp_data_info.nrcur; //R
    cur[1] = otp_data_info.ngrcur; //GR
    cur[2] = otp_data_info.ngbcur; //GB
    cur[3] = otp_data_info.nbcur; //B
} else {
    cur[0] = GOLD_R;
    cur[1] = GOLD_GR;
    cur[2] = GOLD_GB;
...
int GetGoldCode_S5K4H7YX(u16 *gold) {
