...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	} else if (psensor_inst->sensor_idx ==1 && (!strcmp(psensor_inst->psensor_name, "gc5025_mipi_raw"))) {
        /* Yongxia HACK to detect GC5025F for front sensor, since GC5025 & GC5025F share same sensor ID */
		//PK_DBG("Got sensor name as rear sensor ID %x\n", sensorID);
		err = ERROR_SENSOR_CONNECT_FAIL;
	} else {
