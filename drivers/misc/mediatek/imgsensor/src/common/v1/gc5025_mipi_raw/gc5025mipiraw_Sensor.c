...
static int gc5025_gcore_check_version(void)
{
	...
	if((flag_GC5025A & 0x01) == 0x01) {
		...
		LOG_INF("error,This not GC5025A sensor!! \n");
		return 1;
	}
}
            ...
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                   ...
                   if(gc5025_otp_read != 1) {
                       LOG_INF("GC5025:get_imgsensor_id() first time read otp!\n");
            ...
            LOG_INF("gc5025mipiraw_Sensor Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if(sensor_id == GC5025_SENSOR_ID) {
                if(gc5025_otp_read != 1) {
                    LOG_INF("GC5025:open() first time read otp!\n");
                    gc5025_gcore_identify_otp();
                } else {
                    LOG_INF("GC5025:open() otp already read!\n");
                }
            }
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("GC5025:open() i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("GC5025:open() Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
