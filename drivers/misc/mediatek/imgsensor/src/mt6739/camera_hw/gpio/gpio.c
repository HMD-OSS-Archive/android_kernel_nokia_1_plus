...
#ifdef HWID_CHECK_MATCH
extern unsigned short fih_hwid;

struct GPIO_PINCTRL gpio_pinctrl_list_pre_evt[GPIO_CTRL_STATE_MAX_NUM] = {
	/* Main */
	{"cam0_pnd1"},
	{"cam0_pnd0"},
	{"cam0_rst1"},
...
#ifdef HWID_CHECK_MATCH
	if (((fih_hwid >> 4) & 0x00F) == 0) {
        // if ANTMAN pre_evt, the gpio are different with other build.
	    pgpio_pinctrl = gpio_pinctrl_list_pre_evt;
    }
#endif
