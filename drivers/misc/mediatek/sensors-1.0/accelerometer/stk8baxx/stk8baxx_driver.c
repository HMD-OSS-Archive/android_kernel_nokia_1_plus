...
#define STK_ACC_DRIVER_VERSION "FIH.1.6"
#define STK_ACC_DEV_NAME "stk8baxx"
#define STK_ACC_DTS_NAME "mediatek,gsensor"//"mediatek,stk8baxx"
#if 0 /* With int */
...
#define STK8BAXX_RANGESEL_DEF            STK8BAXX_RANGESEL_8G
...
static ssize_t stk_value_show(struct device_driver *ddri, char *buf)
{
    ...
    //stk_read_accel_data(stk);
    gsensor_get_data(&data[0], &data[1], &data[2], &status);
...
#endif /* CONFIG_CUSTOM_KERNEL_SENSORHUB */
    struct stk8baxx_data *stk = stk_data;
    STK_ACC_LOG("delay= %d ms", (int)do_div(delay_ns, 1000));
#ifdef CONFIG_CUSTOM_KERNEL_SENSORHUB
...
#endif /* CONFIG_CUSTOM_KERNEL_SENSORHUB */
    stk_set_delay(stk, (int)do_div(delay_ns, 1000));
    return 0;
}
...
static int stk_factory_set_cali(int32_t data[3])
{
    ...
    STK_ACC_LOG("new x:%d, y:%d, z:%d", data[0], data[1], data[2]);
    xyz[0] = (u8)(data[0]/1000);
    xyz[1] = (u8)(data[1]/1000);
    xyz[2] = (u8)(data[2]/1000);
#if 0
    /* write cali to file */
    error = stk_write_cali_to_file(stk, xyz, STK_K_SUCCESS_FILE);
    ...
    if (error)
    {
        STK_ACC_ERR("stk_set_offset failed!");
        return -1;
    }
    atomic_set(&stk->cali_status, STK_K_SUCCESS_FILE);
    return 0;
