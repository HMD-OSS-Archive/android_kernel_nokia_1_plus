...
static int ltr559_update_als_cali_slope(void)
{
	...
	if(ltr559_obj->als_cali == 0)
	{
		APS_ERR("als_cali is 0!!\n");
		return 0;
	}
...
{
        int  res, on_flag, enable;
        if(!ltr559_obj)
        {
                APS_ERR("ltr559_obj is null!!\n");
                return 0;
        }else if(1 != sscanf(buf, "%d", &on_flag))
        {
        ...
        res = ltr559_ps_enable(ltr559_obj->client, enable);
        if (res < 0)
        {
...
/*--------------------------------------------------------*/
static ssize_t ltr559_show_als_en(struct device_driver *ddri, char *buf)
{
        int len, enable;
        if(!ltr559_obj)
        {
	...
	enable = test_bit(CMC_BIT_ALS, &ltr559_obj->enable);
        len = snprintf(buf, PAGE_SIZE, "%d\n", enable);
        APS_LOG("ltr559_obj->debug_log(%d)\n", enable);
        return len;
}
...
/*--------------------------------------------------------*/
static ssize_t ltr559_store_als_en(struct device_driver *ddri, const char *buf, size_t count)
{
        int err = 0;
	int enable_disable = 0;

        if(1 != sscanf(buf, "%d", &enable_disable))
        {
                APS_ERR("enable_disable= '%d', length = %zu\n", enable_disable, count);
                return -1;
        }
...
static DRIVER_ATTR(ps_en,   S_IWUSR | S_IRUGO, NULL,                    ltr559_store_ps_enable);
...
static int ltr559_ps_factory_set_cali(int32_t als_cali_data[2])
{
