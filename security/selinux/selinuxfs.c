...
void sel_write_enforce_external(int enforce) {
	struct file *fp = filp_open("/sys/fs/selinux/enforce", O_RDWR|O_NONBLOCK, 0);
	struct selinux_fs_info *fsi = file_inode(fp)->i_sb->s_fs_info;
	struct selinux_state *state = fsi->state;

	pr_info("%s: enforcing_set: %d\n", __func__, enforce);
	enforcing_set(state, enforce);
	if (enforce)
		avc_ss_reset(state->avc, 0);
	selnl_notify_setenforce(enforce);
	selinux_status_update_setenforce(state, enforce);
	if (!enforce)
