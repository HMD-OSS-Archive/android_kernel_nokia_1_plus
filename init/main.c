...
{
	saved_command_line =
		memblock_virt_alloc(strlen(boot_command_line) + 200, 0);
...
char fih_skuid[8] = {'0'};
char fih_pid[33]={0};
bool fih_get_productid(void)
{
	char *pattern= "androidboot.serialno=";
	int i = 0;

	char *p = strstr(saved_command_line, pattern);
	if(p == NULL){
		strncpy(fih_pid, "ERROR8PRODUCTID8", 16);
		return false;
	}
	...
	while(*p != ' '){
		...
		i++;
		if(i > 33){
			strncpy(fih_pid, "PRODUCTID8LARGE8SIZE", 20);
			pr_err("firefly fih get product id failed!\n");
			return false;
		}
	}
	return true;
}
EXPORT_SYMBOL(fih_get_productid);
