...
static const char *EXTAMP_MODE_TEXT[] = {"Mode1", "Mode2", "Mode3", "Mode4"};
static const struct soc_enum EXTAMP_MODE_ENUM = SOC_ENUM_SINGLE_EXT(4, EXTAMP_MODE_TEXT);

static int Audio_AuxExtAmpMode_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("\%s n", __func__);
	ucontrol->value.integer.value[0] = extamp_mode;
	return 0;
}
static int Audio_AuxExtAmpMode_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s(), value = %d\n", __func__, ucontrol->value.enumerated.item[0]);
	if (ucontrol->value.integer.value[0] == extamp_mode)
		return 0;
	extamp_mode = ucontrol->value.integer.value[0];
	Ext_Speaker_Amp_Change(true);
	return 0;
}
static const struct snd_kcontrol_new Audio_snd_extamp_controls[] = {
	SOC_ENUM_EXT("Audio EXTAMP mode", EXTAMP_MODE_ENUM, Audio_AuxExtAmpMode_Get, Audio_AuxExtAmpMode_Set),
};
...
static int mt6357_codec_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_add_codec_controls(codec, Audio_snd_extamp_controls,
				   ARRAY_SIZE(Audio_snd_extamp_controls));
